...
export default function ProfitReport({ rows }: { rows: ProfitRow[] }) {
  const totals = rows.reduce(
    (acc, r) => ({
      islened: acc.islened + r.total_islenen,
      odened: acc.odened + r.total_odenen,
      profit: acc.profit + r.profit,
    }),
    { islened: 0, odened: 0, profit: 0 }
  );
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-5">
          ...
          <p className="text-2xl font-bold text-blue-700 mt-1">₺{fmt(totals.islened)}</p>
        </div>
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-5">
          <p className="text-xs font-semibold text-orange-500 uppercase tracking-wide">Toplam Ödenen</p>
          <p className="text-2xl font-bold text-orange-700 mt-1">₺{fmt(totals.odened)}</p>
        </div>
        <div
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {rows.length > 0 ? (
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-right px-6 py-3 font-semibold text-gray-700">Kar (₺)</th>
                <th className="text-center px-6 py-3 font-semibold text-gray-600">Durum</th>
              </tr>
            </thead>
              ...
              {rows.map((row) => {
                const margin =
                  row.total_islenen > 0
                    ...
                    : "0.0";
                return (
                  <tr key={row.site_id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-6 py-4 text-right text-blue-700 font-medium">
                      ₺{fmt(row.total_islenen)}
                    </td>
                    <td className="px-6 py-4 text-right text-orange-600 font-medium">
                      ₺{fmt(row.total_odenen)}
                    </td>
                    <td
          ...
          <div className="text-center py-16 text-gray-400">
            ...
            <p>Henüz makbuz kaydı bulunmuyor.</p>
          </div>
        )}
      </div>
    </div>
